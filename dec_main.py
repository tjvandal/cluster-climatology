...
flags.DEFINE_integer('dae_iters', 2000, 'Number of iterations for pretraining')
flags.DEFINE_integer('sdae_iters', 11001, 'Number of iterations for training autoencoder')
...
flags.DEFINE_string('layer_sizes', '200,200,500,10', 'Sizes of hidden layers')
flags.DEFINE_string('save_dir', '/home/vandal.t/repos/DEC-Bayes/climate-models/', 'Directory to save'\
                    'checkpoints')
...
class DenoisingAE(object):
    def __init__(self, inputs, units, encode_activation=tf.nn.relu,
        ...
        with tf.name_scope("normalize"):
            self.inputs_norm = tf.contrib.layers.batch_norm(inputs)
...
class StackedAutoencoder(object):
    def __init__(self, init_weights, learning_rate=FLAGS.learning_rate):
        ...
        # normalize inputs
        with tf.name_scope("normalize"):
            self.inputs_norm = tf.contrib.layers.batch_norm(self.inputs)
...
def stacked_ae(data):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            encode_data = sess.run(encoder.encoded, feed_dict={encoder.inputs:
                                                               data.next_batch(FLAGS.batch_size*100)[0]})
...
def visualize_clusters(X, memberships=None):
    ...
    plt.scatter(tsne_enc[:,0], tsne_enc[:,1], c=memberships, cmap=plt.cm.jet, alpha=0.5)
...
def train_dec(name='dec'):
    ...
    print "K=%i Clusters" % best_k, init_centriods.shape
    with tf.Graph().as_default():
        with tf.device("/gpu:0"):
        ...
        with tf.Session() as sess:
            ...
            def feed_dict():
                return {dec.inputs: data.next_batch(2000)[0]}

            for i in range(FLAGS.dec_epochs):
                ...
                save_path = saver.save(sess, os.path.join(checkpoint_dir, "model_%08i.ckpt" % i))
            for i in range(50):
                print "Plotting clusters", i
                encoded, memberships = sess.run([dec.encoded, dec.memberships], feed_dict=feed_dict())
                visualize_clusters(encoded, memberships)
            plt.show()
